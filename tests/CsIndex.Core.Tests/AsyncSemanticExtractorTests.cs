...
public sealed class AsyncSemanticExtractorTests
{
    ...
    [Fact]
    public async Task AnalyzeAsync_AssignsDirectAsyncRolesWithoutSyntheticOwnerLeakage()
    {
        const string source = """
            using System;
            using System.Threading.Tasks;
            ...
            await Task.Yield();
            Func<Task> nested = async delegate
            {
                await Task.Yield();
            };
            await nested();
        ...
        var topLevel = Assert.Single(snapshot.Symbols.Values, symbol =>
            symbol.Kind == IndexedSymbolKind.TopLevelStatements);
        var anonymousMethod = Assert.Single(snapshot.Symbols.Values, symbol =>
            symbol.Path?.SegmentKind == CallablePathSegmentKind.AnonymousMethod);
        var bodyless = GetMethod(snapshot, "LoadAsync", "IBodyless");
        var bodylessAccessor = GetMethod(snapshot, "get_Pending", "IBodyless");

        Assert.Equal(AsyncRole.ContainsAwait, topLevel.AsyncRole);
        Assert.Equal(
            AsyncRole.DeclaredAsync | AsyncRole.ReturnsAwaitable | AsyncRole.ContainsAwait,
            anonymousMethod.AsyncRole);
        Assert.Equal(AsyncRole.ReturnsAwaitable, bodyless.AsyncRole);
        Assert.Equal(AsyncRole.ReturnsAwaitable, bodylessAccessor.AsyncRole);
        Assert.Equal("<top-level-statements>.<anonymous-method#1>", anonymousMethod.Path!.ExecutableDisplayPath);
    }
