...
- 永続化: schema/request version 3、`async_role`、`async_involvement_depth`、`async_usage_kind`の保存とDB-only復元を確認する。version mismatchでfail-fastし、既存DBのテーブル、行、journal modeを変更しないことを確認する。`schema_info`のない非空の未認識DBも、marker行を保持し、CSIndexer tableを追加せず、journal modeを変更しないことを確認する。`sqliteXmarker`のようにSQLite内部prefixと似ているだけの有効なユーザーobjectも検出対象に含める。
...
- Storage: verify schema/request-hash version 3, the binding table's primary
  key and foreign keys, both binding indexes, transactional persistence, and
  DB-only reconstruction. Verify that a version 2 database is rejected
